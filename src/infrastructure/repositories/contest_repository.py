...
class ContestRepository(IContestRepository):
    ...
    def update_contest_configuration(self, contest_id: int, rules: Optional[str], rounds_data: List[dict]) -> Optional[Contest]:
        try:
            ...
            self._rollback_session()
            raise e
    def get_contests_by_status(self, status: str) -> List[Contest]:
        try:
            self._rollback_session()
            models = self.session.query(ContestModel).filter_by(status=status).all()
            return [self._to_domain_contest(m) for m in models]
        except Exception:
            self._rollback_session()
            return []
    def get_rounds_by_status(self, status: str) -> List[Round]:
        try:
            self._rollback_session()
            models = self.session.query(RoundModel).filter_by(status=status).all()
            return [self._to_domain_round(m) for m in models]
        except Exception:
            self._rollback_session()
            return []
    def update_contest_status(self, contest_id: int, status: str) -> Optional[Contest]:
        return self.update_contest(contest_id, {'status': status})
    def update_round_status(self, round_id: int, status: str) -> Optional[Round]:
        return self.update_round(round_id, {'status': status})
