    ...
    def _run_ai_detection(
        ...
        try:
            ...
            saved_flag = (
                ...
                .save_ai_flag(
                    submission_id=submission.id,
                    confidence_score=ai_scoreConflict này xảy ra vì hai nhánh đang tiếp cận việc xử lý AI và tìm kiếm ảnh trùng lặp theo hai cách khác nhau:
*   **Nhánh `AIprocessingpipeline` (phần trên `<<<<<<<`):** Đưa việc xử lý AI và tìm ảnh trùng lặp vào một luồng chạy nền (background thread) để không làm chậm quá trình tạo submission của người dùng.
*   **Nhánh `main` (phần dưới `=======`):** Gọi hàm xử lý AI (`_run_ai_detection`) và tìm ảnh trùng lặp (`_run_duplicate_check_and_flag`) chạy đồng bộ (chạy trực tiếp và phải chờ xong mới đi tiếp).

**Cách giải quyết tốt nhất:** 
Bạn nên giữ lại logic chạy ngầm (background thread) của nhánh `AIprocessingpipeline` vì nó sẽ giúp tối ưu hiệu năng và thời gian phản hồi của API. Tuy nhiên, nhánh này đang có một lỗi nhỏ (biến `first_hd_url` chưa được định nghĩa trước khi truyền vào thread) và phần lấy `first_bytes` chưa được chặt chẽ bằng nhánh `main`.

Dưới đây là đoạn code đã được gộp (merge) hoàn chỉnh, kết hợp ưu điểm của cả hai nhánh. Bạn hãy **xóa toàn bộ phần từ `<<<<<<< AIprocessingpipeline` đến `>>>>>>> main`** và thay thế bằng đoạn code sau:
...
```python
        ...
        if status != "draft":
            ...
            # 2. Extract first_bytes for duplicate detection (Combined logic from both branches)
            first_bytes = None
            if files:
                for file_item in files:
                    if self._normalize_file_type(file_item.get("file_type")) == "main_image":
            ...
            # Extract first_hd_url securely
            first_hd_url = first_file.get("image_hd_url") if first_file else None
